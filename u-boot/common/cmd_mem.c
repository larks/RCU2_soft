...
/*
 * New command added for the RCU2
 * This is to write 0x0 for each address in DDR memory 
 * This is necessary to initialize ECC for SECDED. 
 */
int do_mem_mwr (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  ...
  for (;;) {
    ...
    if (iteration_limit && iterations > iteration_limit) {
      printf("Done writing the pattern from 0x%x to 0x%x exit....\n", start, end);
      return 0;
    }
    ++iterations;
...
int do_mem_mtest (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	for (;;) {
		...
		for (offset = 1; offset < len; offset <<= 1) {
		    ...
		    if (temp != pattern) {
		      printf ("\nFAILURE: Address bit stuck high @ 0x%.8lx:"
			      " expected 0x%.8lx, actual 0x%.8lx (offset=0x%x)\n",
			      (ulong)&start[offset], pattern, temp, offset);
		      errs++;
		      if (ctrlc()) {
			putc ('\n');
			return 1;
		...
		for (test_offset = 1; test_offset < len; test_offset <<= 1) {
			...
			if ((temp != pattern) && (offset != test_offset)) {
			  printf ("\nFAILURE: Address bit stuck low or shorted @"
				  " 0x%.8lx: expected 0x%.8lx, actual 0x%.8lx (offset=0x%x)\n",
				  (ulong)&start[offset], pattern, temp, offset);
			  errs++;
			  if (ctrlc()) {
			    putc ('\n');
		 ...
		 * Returns:     0 if the test succeeds, 1 if the test fails.
		 */
		num_words = ((ulong)end - (ulong)start)/sizeof(vu_long) + 0; // rcu2
...
U_BOOT_CMD(
	...
	"complete or quick RAM read/write test",
