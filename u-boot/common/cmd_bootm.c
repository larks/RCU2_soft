...
static int bootm_start(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  ...
  puts("Linux image is OK \n");
  /*
  char *s;
  u8 index=2;
  s = getenv("bootcmd");
  if(strcmp(s,"run flashboot")==0){
    index = 0;
  }else if(strcmp(s,"run netboot")==0){
    index = 1;
  }
  */

  MSMVERSION->linux_version = (linux_boot_index << 4) + uboot_pars_from_flash;
  ///linux version
  ///last 2 word
  /// 12 --> netboot and default uboot
  /// 10 --> netboot and SPI0.0
  /// 11 --> netboot and SPI0.1
  /// 00 --> flashboot and SPI0.0
  /// 01 --> flashboot and SPI0.1

  
#ifdef CONFIG_USE_SPI_FLASH_2
  MSMVERSION->uboot_version = (0x01121D00 + uboot_pars_from_flash);
#else
  MSMVERSION->uboot_version = (0x01120D00 + uboot_pars_from_flash);
#endif
...
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  ...
  if (bootm_start(cmdtp, flag, argc, argv)){
    puts(" bootm_start failed...\n");
    /*
    puts(" bootm_start failed.... change bootcmnd...\n");
    char *s;
    s = getenv("bootcmd");
    if(strcmp(s,"run flashboot")==0){
