...
int do_wrlv(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  u8 option = 0;
  if(strcmp(argv[1],"0")==0){
     option = 0;
     printf("Linux is from SPI Flash\n");
  }else if(strcmp(argv[1],"1")==0){
    option = 1;
    printf("Linux is from Netboot\n");
  }    
  linux_boot_index = option;
}

U_BOOT_CMD(
	   wrlv, 2, 1,	do_wrlv,
	   "write linux_boot_index (0:Flash, 1:Netboot)",
	   ""
);
...
int do_loadenv (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  ...
  if(argc!=2){
    printf("Usage: loadenv [0,1,2]\n");
    return 1;
  }else{
    if(strcmp(argv[1],"0")==0){
      option = 0;
      printf("Loading uBoot environments from SPI0.0\n");
    }else if(strcmp(argv[1],"1")==0){
      option = 1;
      printf("Loading uBoot environments from SPI0.1\n");
    }else if(strcmp(argv[1],"2")==0){
      option = 2;
      printf("Loading uBoot environments from default\n");
    }else{
      printf("loadenv [0,1,2]\n");
      return 1;
    }
  }
  return (loadenv(option) ? 1 : 0);
}

U_BOOT_CMD(
	   loadenv, 2, 1,	do_loadenv,
