...
int do_cptf(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	g_init_envm_cr = M2S_SYSREG->envm_cr ;
	M2S_SYSREG->envm_cr = (g_init_envm_cr & NVM_FREQRNG_MASK) | NVM_FREQRNG_MAX;
	...
	Done:
	M2S_SYSREG->envm_cr = g_init_envm_cr;
