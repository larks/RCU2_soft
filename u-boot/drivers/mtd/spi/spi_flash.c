...
int spi_flash_cmd_m2s(struct spi_slave *spi, u8 cmd, void *response, size_t len)
{
  int ret;
  
  ret = MSS_SPI_transfer_block(spi, &cmd, sizeof(cmd), response, len);
...
int spi_flash_read_m2s(struct spi_flash *flash, const u8 *cmd,
		       size_t cmd_len, void *data, size_t data_len)
{
  struct spi_slave *spi = flash->spi;
  int ret ;
  //printf(" spi_flash_read_m2s\n");
  ret = MSS_SPI_transfer_block(spi, cmd, cmd_len, data, data_len);
  
  return ret;
}


int spi_flash_write_m2s(struct spi_flash *flash, const u8 *cmd,
			size_t cmd_len, void *data, size_t data_len)
{
  struct spi_slave *spi = flash->spi;
  int ret ;
  
  ret = MSS_SPI_transfer_block(spi, cmd, cmd_len, 0, 0);
...
void set_slave_select(struct spi_flash *flash){
  MSS_SPI_set_slave_select(flash->spi);
}
void clear_slave_select(struct spi_flash *flash){
