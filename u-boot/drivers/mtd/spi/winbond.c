...
#define CMD_W25_VOLATILE_WREN	0x50	/* Write Enable */
#define CMD_W25_WRDI		0x04	/* Write Disable */
#define CMD_W25_RDSR		0x05	/* Read Status Register */
#define CMD_W25_RDSR2		0x35	/* Read Status Register */
...
#define CMD_W25_ENRS            0x66
#define CMD_W25_RST             0x99

#define WINBOND_ID_W25X16		0x4015
#define WINBOND_ID_W25X32		0x4016
...
static int winbond_wait_ready(struct spi_flash *flash, unsigned long timeout)
{
  ...
  do {
    ret = spi_flash_cmd(spi, CMD_W25_RDSR, &status, sizeof(status));
    //ret = spi_flash_cmd_m2s(spi, CMD_W25_RDSR, &status, sizeof(status));

    if (ret)
      return -1;

    if ((status & WINBOND_SR_WIP) == 0){
      printf(" winbond_wait_ready : status register = 0x%x\n", status);
      break;
    }
		...
		u32 offset, size_t len, void *buf)
{
  ...
  set_slave_select(flash);
  winbond_wait_ready(flash, SPI_FLASH_PROG_TIMEOUT);
  ret = spi_flash_read_m2s(flash, cmd, sizeof(cmd), buf, len);
  winbond_wait_ready(flash, SPI_FLASH_PROG_TIMEOUT);
  clear_slave_select(flash);
...
int winbond_erase(struct spi_flash *flash, u32 offset, size_t len)
{
	...
	ret = spi_flash_cmd(flash->spi, CMD_W25_RDSR, &status, sizeof(status));
	...
	//ret = spi_flash_cmd(flash->spi, CMD_W25_WREN, NULL, 0);
	//ret = spi_flash_cmd(flash->spi, CMD_W25_VOLATILE_WREN, NULL, 0);

	//udelay(100000);
	//printf("reset the chip \n");
	//ret = spi_flash_cmd(flash->spi, CMD_W25_ENRS, &status, sizeof(status));	
	//udelay(100000);
	//ret = spi_flash_cmd(flash->spi, CMD_W25_RST, &status, sizeof(status));	
	//udelay(100000);
	...
	printf(" Write status register = 0x%x 0x%x\n", cmd[1], cmd[2]);
	ret = winbond_wait_ready(flash, SPI_FLASH_PAGE_ERASE_TIMEOUT);	
