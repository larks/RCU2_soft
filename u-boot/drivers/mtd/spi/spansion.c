...
#define CMD_S25FLXX_RDSR	0x05	/* Read Status Register1 */
#define CMD_S25FLXX_RDSR2	0x35	/* Read Status Register2 */
#define CMD_S25FLXX_RDSR3	0x33	/* Read Status Register3 */
...
static int spansion_wait_ready(struct spi_flash *flash, unsigned long timeout)
{
  ...
  do {
    ...
    if ((status & SPANSION_SR_WIP) == 0){
      //printf(" spansion_wait_ready : status register = 0x%x\n", status);
      //ret = spi_flash_cmd(spi, 0xAB, &code, sizeof(code));
      //printf(" code:  %x %x %x %x %x\n",code[0], code[1], code[2], code[3], code[4]);
      //printf(" spansion_wait_ready : status register2 = 0x%x\n", status2);
      //ret = spi_flash_cmd(spi, CMD_S25FLXX_RDSR3, &status3, sizeof(status3));
      //printf(" spansion_wait_ready : status register3 = 0x%x\n", status3);
      break;
    }
...
int spansion_erase(struct spi_flash *flash, u32 offset, size_t len)
{
  ...
  /*
  ret = spi_flash_cmd(flash->spi, CMD_S25FLXX_RDSR, &status, sizeof(status));
