...
static inline int spi_m2s_hw_clk_set(struct m2s_spi_slave *s, unsigned int spd)
{
  ...
  //printf("bus=%d,cnt_hz=%d,slv_hz=%d,rsl_hz=%d,clk_gen=%d,ret=%d\n",
				  ...
				  unsigned int hz, unsigned int m)
{
	...
	if (!s) {
	  printf("No malloc for s m2s_spi_slave. size=%d\n", sizeof(struct m2s_spi_slave));
	  goto done;
	}else{
	  printf("malloc is ok\n");
	}
	...
	printf("s=%p, slv=%p bus=%d,regs=%p,cs=%d,hz=%d,mode=0x%x\n", 
	       s, slv, b, s->regs, cs, hz, m);
...
int spi_claim_bus(struct spi_slave *slv)
{
  ...
  //do{
  //rst_bit = (MSS_SPI(s)->control & SPI_CONTROL_RESET) >> 31;
  //}while(rst_bit==1);
  //printf(" --- control = 0x%x \n",  MSS_SPI(s)->control);
  //printf(" --- control = 0x \n");
  //udelay(10000);
  ...
  ////from softconsole 
  //MSS_SPI(s)->control  &= ~(uint32_t)0x00000001;
  //MSS_SPI(s)->control  |=  (uint32_t)0x00000002;
  //MSS_SPI(s)->control  |=  (uint32_t)0x00000001;
  ...
  /*
    printf("bus=%d,soft_reset_cr=0x%x,control=0x%x\n",
    slv->bus, M2S_SYSREG->soft_reset_cr, MSS_SPI(s)->control);
  */
...
void MSS_SPI_set_slave_select(struct spi_slave *slv){
  
  //printf("MSS_SPI_set_slave_select \n");
  struct m2s_spi_slave *s = to_m2s_spi(slv);
  spi_claim_bus(slv);
  spi_m2s_hw_bt_set(s, 8);
  spi_m2s_hw_clk_set(s, s->hz);
  spi_m2s_hw_cs_set(s, s->slave.cs);
  spi_m2s_hw_mode_set(s, s->mode);
  //  printf("end of MSS_SPI_set_slave_select \n");

}


static void recover_from_rx_overflow(struct m2s_spi_slave *s){
  ...
  control_reg = MSS_SPI(s)->control;
  clk_gen = MSS_SPI(s)->clk_gen;
  frame_size = MSS_SPI(s)->txrxdf_size;
  control2 = MSS_SPI(s)->control2;
  packet_size = MSS_SPI(s)->pktsize;
  cmd_size = MSS_SPI(s)->cmdsize;
  slave_select = MSS_SPI(s)->slave_select;
  ...
  MSS_SPI(s)->control &= ~(0x80000000u);
  

  /*
   * Restore SPI hardware block configuration.
   */
  control_reg &= ~(uint32_t)0x00000001u; //CTRL_ENABLE_MASK;
  MSS_SPI(s)->control = control_reg;
  MSS_SPI(s)->clk_gen = clk_gen;
  MSS_SPI(s)->txrxdf_size = frame_size;
  MSS_SPI(s)->control |= 0x00000001u; //CTRL_ENABLE_MASK;
  MSS_SPI(s)->control2 = control2;
  MSS_SPI(s)->pktsize = packet_size;
  MSS_SPI(s)->cmdsize = cmd_size;
  MSS_SPI(s)->slave_select = slave_select;

}


int MSS_SPI_transfer_block(struct spi_slave *slave, 
  ...
  //printf("MSS_SPI_transfer_block cmd=0x%x\n", cmd_buffer[0]);

  uint8_t *rd_buffer = (uint8_t*)buf;
  ...
  struct m2s_spi_slave *s = to_m2s_spi(slave);
  ...
  MSS_SPI(s)->command |= (0x00000008u | 0x00000004u);

  /* Recover from receive overflow. */
  rx_overflow = MSS_SPI(s)->status & 0x00000004u;
  if(rx_overflow){
    recover_from_rx_overflow(s);
  }

  /* Set frame size to 8 bits and the frame count to the transfer size. */
  MSS_SPI(s)->control &= ~(uint32_t)0x00000001u;
  MSS_SPI(s)->control = (MSS_SPI(s)->control & ~(0x00FFFF00u)) | ( (frame_count << 8) & 0x00FFFF00u);
  MSS_SPI(s)->txrxdf_size = 8 ; //MSS_SPI_BLOCK_TRANSFER_FRAME_SIZE;
  MSS_SPI(s)->control |= 0x00000001u; 
  
  /* Flush the receive FIFO. */
  rx_fifo_empty = MSS_SPI(s)->status & 0x00000040u; //RX_FIFO_EMPTY_MASK;
  while(0u == rx_fifo_empty){
    rx_raw = MSS_SPI(s)->rx_data;
    rx_fifo_empty = MSS_SPI(s)->status & 0x00000040u;
  }
  ...
  while(transfer_idx < transfer_size){
	...
	if(rx_idx < rd_byte_size){
	  rd_buffer[rx_idx] = rx_raw & 0xFF; //(uint8_t)rx_raw;   
	}
	++rx_idx;
      }
      ++transfer_idx;
      --transit;
      tx_fifo_full = MSS_SPI(s)->status & 0x00000100u; //TX_FIFO_FULL_MASK;
      if(0u == tx_fifo_full){
	if(transit < 4){ //RX_FIFO_SIZE){
	  /* Send another byte. */
	  if(tx_idx < cmd_byte_size){
	    MSS_SPI(s)->tx_data = cmd_buffer[tx_idx];
	    ++tx_idx;
  ...
  //printf("end of MSS_SPI_transfer_block \n");
  return 0;
