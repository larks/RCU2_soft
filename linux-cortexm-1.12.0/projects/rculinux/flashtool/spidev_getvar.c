...
static char var_name[32] = "serverip";
...
void dump_data_buffer(void){
  ...
  ///seek for the variable pattern 
  match = 0;
  for(i=0;i<size-strlen(var_name);i++){
    if(data_buffer[i] != var_name[0]){
      continue;
    }

    match = 1;
    for(j=0;j<strlen(var_name); j++){
      if(data_buffer[i+j] == var_name[j]){
	match = match*1;
      }else{
	match = 0;
      }
    ...
    if(match==1){
      for(j=0;j<strlen(var_name)+100; j++){
	if(data_buffer[i+j]==0x00){
	  break;
	}else{
	  printf("%c", data_buffer[i+j]);
	}
      }
      printf("\n");
    }
...
int main(int argc, char *argv[])
{
  ...
  close(fd);
  free(env);
  printf("spidev_getvar is successfully done.\n");
  return ret;
}
