...
static char action[32] = "ISP_PROG_SIZE";
...
void write_data_buffer(char action[], int val){
  ...
  for(i=0;i<size-14-strlen(char_val);i++){
    //bootcmd=run 
    if(data_buffer[i]==C_I &&  
       data_buffer[i+1]==C_S && 
       data_buffer[i+2]==C_P && 
       data_buffer[i+3]==C_ub && 
       data_buffer[i+4]==C_P && 
       data_buffer[i+5]==C_R && 
       data_buffer[i+6]==C_O && 
       data_buffer[i+7]==C_G && 
       data_buffer[i+8]==C_ub && 
       data_buffer[i+9]==C_S && 
       data_buffer[i+10]==C_I && 
       data_buffer[i+11]==C_Z && 
       data_buffer[i+12]==C_E && 
       data_buffer[i+13]==C_eq){

      bootcmd_start_addr = i;
      
      for(j=0;j<10;j++){
	printf("%02d", data_buffer[i+14+j]);
	if(data_buffer[i+14+j]==0){
	  addr_offset = j;
	  break;
	}
      }
      ...
      ///zero-th
      for(j=0;j<addr_offset;j++){
	data_buffer[i+14+j]=0;
      }
      printf("\n");
      for(j=0;j<strlen(char_val);j++){
	data_buffer[i+14+j] = char_val[j];
	printf("%02d", data_buffer[i+14+j]);
      }
      printf("\n");
    }
...
{
  printf("Usage: %s ISP_PROG_SIZE [integer(filesize)]\n", prog);
  exit(1);
}
...
void parse_opts(int argc, char *argv[])
{
  ...
  if(argc!=3){
    ...
    if(strcmp(action,"ISP_PROG_SIZE")!=0){
      print_usage(argv[0]);
    }else{
...
int main(int argc, char *argv[])
{
  ...
  write_data_buffer(action, filesize);
  if(do_writing){
  ...
  close(fd);
  free(env);
  printf("spidev_chenv is successfully done.\n");
  return ret;
}
