...
void write_data_buffer(char action[]){
  ...
  for(i=0;i<size-21;i++){
    ...
    if(data_buffer[i]==C_b &&  
       ...
       data_buffer[i+5]==C_m && 
       data_buffer[i+6]==C_d && 
       data_buffer[i+7]==C_eq&& 
       data_buffer[i+8]==C_r && 
       data_buffer[i+9]==C_u && 
       data_buffer[i+10]==C_n && 
       data_buffer[i+11]==C_sp){

      bootcmd_start_addr = i;

      ///flashboot --> netboot
      if(data_buffer[i+12] == C_f &&  
	 data_buffer[i+13] == C_l && 
	 data_buffer[i+14] == C_a && 
	 data_buffer[i+15] == C_s && 
	 data_buffer[i+16] == C_h && 
	 data_buffer[i+17] == C_b && 
	 data_buffer[i+18] == C_o && 
	 data_buffer[i+19] == C_o && 
	 data_buffer[i+20] == C_t 
	 ){
	
	if(strcmp(action,"flashboot")==0){
	  printf("bootcmd is already flashboot. Do nothing...\n");
	  do_writing = 0;
	  addr_offset = 0;
	}else{
	  addr_offset = 2;
	...
	break;
      ...
      }else if(data_buffer[i+12] == C_n && 
	       data_buffer[i+13] == C_e && 
	       data_buffer[i+14] == C_t && 
	       data_buffer[i+15] == C_b && 
	       data_buffer[i+16] == C_o && 
	       data_buffer[i+17] == C_o && 
	       data_buffer[i+18] == C_t
	       ){ //netboot --> flasboot
	if(strcmp(action,"netboot")==0){
	  printf("bootcmd is already netboot. Do nothing...\n");
	  do_writing = 0;
	  addr_offset = 0;
	}else{
	  addr_offset = 2;
  ...
  for(i=0; i<bootcmd_start_addr;i++){
    data_out_buffer[i] = data_buffer[i];
  }
  for(i=bootcmd_start_addr; i<bootcmd_start_addr+12; i++){ //write bootcmd=run 
    data_out_buffer[i] = data_buffer[i]; //bootcmd=run 
  }
   
  if(strcmp(action,"netboot")==0){ //flashboot --> netboot
    data_out_buffer[bootcmd_start_addr+12] = C_n; 
    data_out_buffer[bootcmd_start_addr+13] = C_e; 
    data_out_buffer[bootcmd_start_addr+14] = C_t; 
    data_out_buffer[bootcmd_start_addr+15] = C_b; 
    data_out_buffer[bootcmd_start_addr+16] = C_o; 
    data_out_buffer[bootcmd_start_addr+17] = C_o; 
    data_out_buffer[bootcmd_start_addr+18] = C_t;

    for(i=bootcmd_start_addr+19;  i<size-addr_offset; i++){
      data_out_buffer[i] = data_buffer[i+addr_offset];
    }
    //for(i=size-addr_offset ; i<size; i++){
    //data_out_buffer[i] = C_sp;
    // }

   }else if(strcmp(action,"flashboot")==0){ //flashboot
     data_out_buffer[bootcmd_start_addr+12] = C_f; 
     data_out_buffer[bootcmd_start_addr+13] = C_l; 
     data_out_buffer[bootcmd_start_addr+14] = C_a; 
     data_out_buffer[bootcmd_start_addr+15] = C_s; 
     data_out_buffer[bootcmd_start_addr+16] = C_h; 
     data_out_buffer[bootcmd_start_addr+17] = C_b; 
     data_out_buffer[bootcmd_start_addr+18] = C_o; 
     data_out_buffer[bootcmd_start_addr+19] = C_o; 
     data_out_buffer[bootcmd_start_addr+20] = C_t; 

     for(i=bootcmd_start_addr+21;  i<size; i++){
       data_out_buffer[i] = data_buffer[i-addr_offset];
     }
...
{
  printf("Usage: %s [netboot/flashboot]\n", prog);
  exit(1);
}
...
int main(int argc, char *argv[])
{
  ...
  close(fd);
  free(env);
  printf("spidev_bootcmd is successfully done.\n");
  return ret;
}
