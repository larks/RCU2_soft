...
)
{
    ...
    else
    {
      ...
      device_version = SYSREG->DEVICE_VERSION;
	    ...
	    //printf("start writing status = 0x%x...\n", status);
			...
			//printf(" ok. keep going...rest = 0x%x\n", remaining_length);
    ...
    //printf("End of NVM_write: written_size = 0x%x. target_size=0x%x\n", written_size, length);
...
)
{
    ...
    else
    {
        ...
        if((nvm_offset + length) <= MAX_512K_OFFSET)
        {
		  ...
		  //printf(" page = %d\n", current_page);
		    ...
		    if(MSS_ENVM->status & MSS_NVM_WR_DENIED)
                    {
                        /* Clear the access denied flag */
		      //g_nvm[block]->CLRHINT |= ACCESS_DENIED_FLAG_CLEAR;
		      MSS_ENVM->clrhint |= ACCESS_DENIED_FLAG_CLEAR;        
                    }
		      ...
		      MSS_ENVM->wdbuff[inc] = p_nvm32[inc];
                    }

		    //printf(" wdbuff done and user_unlock\n");

                    //g_nvm[block]->PAGE_LOCK = NVM_DO_NOT_LOCK_PAGE;
                    //g_nvm[block]->CMD = USER_UNLOCK | (current_offset & PAGE_ADDR_MASK);
		    MSS_ENVM->page_lock_set = NVM_DO_NOT_LOCK_PAGE;
		    MSS_ENVM->cmd = USER_UNLOCK | (current_offset & PAGE_ADDR_MASK);

                    /* Issue program command */
                    //g_nvm[block]->CMD = PROG_ADS | (current_offset & PAGE_ADDR_MASK);
		    MSS_ENVM->cmd = PROG_ADS | (current_offset & PAGE_ADDR_MASK);     

                    /* Wait for NVM to become ready. */
		    //printf(" wait for NVM to become ready\n");
                    ctrl_status = wait_nvm_ready(block);
		    //printf(" ctrl_status = 0x%x\n", ctrl_status);
                    /* Check for errors. */
    ...
    //printf(" End of NVM_unlock return code = 0x%x\n", status);
...
static nvm_status_t request_nvm_access(uint32_t nvm_block_id)
{
     ...
     */
    timeout_counter = 160000000u / 16u;
    ...
    */
    ...
    MSS_ENVM->reqaccess = REQUEST_NVM_ACCESS;
    while (MSS_ENVM->reqaccess != CORTEX_M3_ACCESS_GRANTED);
...
static nvm_status_t get_ctrl_access(uint32_t nvm_offset, uint32_t length)
{
    ...
    //printf("End of get_ctrl_access. return code = 0x%x\n", access_req_status);
  ...
  if(offset <= NVM1_TOP_OFFSET)
    {
      ...
      //printf("aaaa length = 0x%x, offset=0x%x, block=0x%x\n", length_written, offset, block);
      ...
      MSS_ENVM->page_lock_set = lock_page;
      
      /* Issue program command */
      //g_nvm[block]->CMD = PROG_ADS | (offset & PAGE_ADDR_MASK);
      MSS_ENVM->cmd =  PROG_ADS | (offset & PAGE_ADDR_MASK);    
	  ...
	  *p_status = MSS_ENVM->status;
	  ...
	  MSS_ENVM->cmd = VERIFY_ADS | (offset & PAGE_ADDR_MASK);        
	  /* Wait for NVM to become ready. */
	  ctrl_status = wait_nvm_ready(block);
	  ...
	  if(errors){
	    ...
	    //*p_status = g_nvm[block]->STATUS;
	    *p_status = MSS_ENVM->status;
	    //printf("error in verifing the envm contents at addr=0x%x\n", addr);
	  }else{
	    //printf("verifing the envm contents is ok.... addr = 0x%x\n", addr);
	  }
        }
    }
 ...
 )
{
  ...
  if(length != BYTES_PER_PAGE)
    {
	  ...
	  MSS_ENVM->wdbuff[first_non_written_word + inc] = p_nvm32[first_non_written_word + inc]; 
        }
    }
  ...
  for(inc = 0u; inc < length; inc=inc+4)
    {
      //printf("inc=%d, 0x%x\n", inc, p_data[inc]);
      //g_nvm[block]->WD[wd_offset + inc] = p_data[inc];
      MSS_ENVM->wdbuff[wd_offset + inc/4] = p_data[inc]<<24 + p_data[inc+1]<<16 
	+ p_data[inc+2]<<8 + p_data[inc];     
    }
}
...
int main(int argc, char **argv) {
  ...
  printf("dst=0x%x, File_size = 0x%x\n", dst, file_size);
  ...
  */
  ...
  uint32_t unit_size = 128;
  uint32_t total_size = 0;
  uint32_t rd_size = 0;

  data_buffer = (uint8_t*) malloc(unit_size);

  while(total_size < file_size){
    if(total_size-file_size<unit_size){
      rd_size = total_size-file_size;
    }else{
      rd_size = unit_size;
    }

    fseek(infile, total_size, SEEK_SET);
    fread(data_buffer, sizeof(uint8_t), rd_size, infile);
  
    //status = NVM_unlock(dst, file_size);
    status = NVM_unlock(dst+total_size, rd_size);

    if(NVM_SUCCESS == status){
      status = NVM_write(dst+total_size, data_buffer, rd_size, NVM_LOCK_PAGE);
      if(NVM_SUCCESS == status){
	//printf("cptf ok..status = NVM_SUCCESS... keep going... size=0x%x\n", total_size);
	success = 1; 
      }else{
	printf("cptf write failed...break...size=0x%x. error_code=%d\n", total_size, status);
	success = 0;
	//break;
      }
    }else{
      printf("NVM_unlock failed... break...size=0x%x. error_code=%d\n", total_size, status);
      success = 0;
      //break;
    }
  ...
  printf("total_size_written = 0x%x\n", total_size);
  free(data_buffer);
  return success;
}
