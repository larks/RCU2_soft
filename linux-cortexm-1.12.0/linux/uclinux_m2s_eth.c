...
static __init int m2s_mac_hw_init(struct m2s_mac_dev *d)
{
  ...
  */
  ...
  M2S_MAC_CFG(d)->cfg1 = 0; // stop the early irq
...
int m2s_mac_rx_packets(struct m2s_mac_dev *d, int limit)
{
  ...
  char *prx;
    ...
    /*
    debug("%s, size = %04x, data: ", __func__, size);
    prx = d->rx.buf[idx];
    for (i=0; i<32; i++) 
    {
      debug("%02x ", (int)prx[i]);
...
static int m2s_mac_net_open(struct net_device *dev)
{
  ...
  u32 txs, rxs;
	...
	/*
	 * Clear the TxPktCount and RxPktCount
   * this requires for the proper interrupt handling
	 */
  for (;;) {
    txs = M2S_MAC_DMA(d)->tx_stat;
    rxs = M2S_MAC_DMA(d)->rx_stat;
    //debug("  0x%08x 0x%08x\n", txs, rxs);
    if (txs & 0x00FF0000) {
      M2S_MAC_DMA(d)->tx_stat = M2S_MAC_DMA_STAT_TX_PS;
    }
    ...
    if (((txs & 0x00FF0000) == 0) && ((rxs & 0x00FF0000) == 0)) {
      break;
    }
  }
...
static int __init m2s_mac_probe(struct platform_device *pd)
{
  ...
  debug("%s: M2S_TX_NUM = %d\n", __func__, M2S_TX_NUM);
  debug("%s: M2S_RX_NUM = %d\n", __func__, M2S_RX_NUM);
