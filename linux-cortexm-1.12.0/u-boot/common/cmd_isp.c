...
#define BUFFER_A_SIZE  512 //512 //1024
...
static void exec_spi_flash_probe(u32 speed)
{
  ...
  /*
  struct m2s_spi_slave *s = container_of(slave, struct m2s_spi_slave, slave); //to_m2s_spi(spi_flash->spi);
  printf("MSS SPI %p\n", MSS_SPI(s));
  printf(" --- control = 0x%x \n", MSS_SPI(s)->control);
...
static int exec_spi_flash_read(long offset)
{
  ...
  buf = (u8 *)malloc(len);
  //buf = map_physmem(g_flash_rd_buf, len, MAP_WRBACK);
  if (!buf) {
